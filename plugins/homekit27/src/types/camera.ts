...
addSupportedType({
    ...
    async getAccessory(device: ScryptedDevice & VideoCamera & VideoCameraConfiguration & Camera & MotionSensor & AudioSensor & Intercom & OnOff, homekitPlugin: HomeKitPlugin) {
        ...
        try {
            ...
            if (streamOptions?.some(o => o.video?.codec === 'h265')) {
                isHevc = true;
                console.log('HomeKit 27: H.265 (HEVC) natively supported by camera, advertising HEVC to HomeKit.');
            }
        } catch (e) {
            console.error('Failed to get video stream options for HEVC check', e);
        }
        ...
        if (!forceOpus) {
            ...
            // Apple supports AAC_LC
            enabledStreamingCodecTypes.push(AudioStreamingCodecType.AAC_LC);
        }
        ...
        const streamingOptions: CameraStreamingOptions = {
            video: {
                codec: {
                    ...
                    type: isHevc ? VideoCodecType.H265 : VideoCodecType.H264,
                },
