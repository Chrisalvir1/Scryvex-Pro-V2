...
class ChatCompletionChunk(TypedDict):
    ...
    choices: list[Choice]  # A list of chat completion choices. Can contain more than one elements if   is greater than 1. Can also be empty for the last chunk if you set  .
    created: float  # The Unix timestamp (in seconds) of when the chat completion was created. Each chunk has the same timestamp.
    ...
    moderation: Moderation | Any  # Moderation results for the request input and generated output. Present on the moderation chunk when moderated completions are requested.
    object: Any  # The object type, which is always  .
    service_tier: Any | Any | Any | Any | Any | Any  # Specifies the processing type used for serving the request.  - If set to 'auto', then the request will be processed with the service tier   configured in the Project settings. Unless otherwise configured, the Project   will use 'default'. - If set to 'default', then the request will be processed with the standard   pricing and performance for the selected model. - If set to '[flex](https://platform.openai.com/docs/guides/flex-processing)' or   '[priority](https://openai.com/api-priority-processing/)', then the request   will be processed with the corresponding service tier. - When not set, the default behavior is 'auto'.  When the   parameter is set, the response body will include the   value based on the processing mode actually used to serve the request. This response value may be different from the value set in the parameter.
    system_fingerprint: str
    usage: CompletionUsage | Any  # An optional field that will only be present when you set   in your request. When present, it contains a null value **except for the last chunk** which contains the token usage statistics for the entire request.  **NOTE:** If the stream is interrupted or cancelled, you may not receive the final usage chunk which contains the total token usage for the request.

class ChatCompletionCreateParamsNonStreaming(TypedDict):

    audio: ChatCompletionAudioParam | Any  # Parameters for audio output. Required when audio output is requested with  . [Learn more](https://platform.openai.com/docs/guides/audio).
    frequency_penalty: float | Any  # Number between -2.0 and 2.0. Positive values penalize new tokens based on their existing frequency in the text so far, decreasing the model's likelihood to repeat the same line verbatim.
    function_call: Any | Any | ChatCompletionFunctionCallOption
    functions: list[Function]
    logit_bias: Any | Any  # Modify the likelihood of specified tokens appearing in the completion.  Accepts a JSON object that maps tokens (specified by their token ID in the tokenizer) to an associated bias value from -100 to 100. Mathematically, the bias is added to the logits generated by the model prior to sampling. The exact effect will vary per model, but values between -1 and 1 should decrease or increase likelihood of selection; values like -100 or 100 should result in a ban or exclusive selection of the relevant token.
    logprobs: bool | Any  # Whether to return log probabilities of the output tokens or not. If true, returns the log probabilities of each output token returned in the   of  .
    max_completion_tokens: float | Any  # An upper bound for the number of tokens that can be generated for a completion, including visible output tokens and [reasoning tokens](https://platform.openai.com/docs/guides/reasoning).
    max_tokens: float | Any
    ...
    metadata: Metadata | Any  # Set of 16 key-value pairs that can be attached to an object. This can be useful for storing additional information about the object in a structured format, and querying for objects via API or the dashboard.  Keys are strings with a maximum length of 64 characters. Values are strings with a maximum length of 512 characters.
    modalities: list[Any | Any] | Any  # Output types that you would like the model to generate. Most models are capable of generating text, which is the default:     The   model can also be used to [generate audio](https://platform.openai.com/docs/guides/audio). To request that this model generate both text and audio responses, you can use:  
    model: Union[str, Any] | ChatModel  # Model ID used to generate the response, like   or  . OpenAI offers a wide range of models with different capabilities, performance characteristics, and price points. Refer to the [model guide](https://platform.openai.com/docs/models) to browse and compare available models.
    moderation: Moderation | Any  # Configuration for running moderation on the request input and generated output.
    n: float | Any  # How many chat completion choices to generate for each input message. Note that you will be charged based on the number of generated tokens across all of the choices. Keep   as   to minimize costs.
    ...
    prediction: ChatCompletionPredictionContent | Any  # Static predicted output content, such as the content of a text file that is being regenerated.
    presence_penalty: float | Any  # Number between -2.0 and 2.0. Positive values penalize new tokens based on whether they appear in the text so far, increasing the model's likelihood to talk about new topics.
    ...
    prompt_cache_retention: Any | Any | Any  # The retention policy for the prompt cache. Set to   to enable extended prompt caching, which keeps cached prefixes active for longer, up to a maximum of 24 hours. [Learn more](https://platform.openai.com/docs/guides/prompt-caching#prompt-cache-retention). For  ,  , and future models, only   is supported.  For older models that support both   and  , the default depends on your organization's data retention policy:  - Organizations without ZDR enabled default to  . - Organizations with ZDR enabled default to   when     is not specified.
    reasoning_effort: ReasoningEffort  # Constrains effort on reasoning for [reasoning models](https://platform.openai.com/docs/guides/reasoning). Currently supported values are  ,  ,  ,  ,  , and  . Reducing reasoning effort can result in faster responses and fewer tokens used on reasoning in a response.  -   defaults to  , which does not perform reasoning. The supported   reasoning values for   are  ,  ,  , and  . Tool   calls are supported for all reasoning values in gpt-5.1. - All models before   default to   reasoning effort, and do not   support  . - The   model defaults to (and only supports)   reasoning effort. -   is supported for all models after  .
    ...
    seed: float | Any
    service_tier: Any | Any | Any | Any | Any | Any  # Specifies the processing type used for serving the request.  - If set to 'auto', then the request will be processed with the service tier   configured in the Project settings. Unless otherwise configured, the Project   will use 'default'. - If set to 'default', then the request will be processed with the standard   pricing and performance for the selected model. - If set to '[flex](https://platform.openai.com/docs/guides/flex-processing)' or   '[priority](https://openai.com/api-priority-processing/)', then the request   will be processed with the corresponding service tier. - When not set, the default behavior is 'auto'.  When the   parameter is set, the response body will include the   value based on the processing mode actually used to serve the request. This response value may be different from the value set in the parameter.
    stop: str | list[str] | Any  # Not supported with latest reasoning models   and  .  Up to 4 sequences where the API will stop generating further tokens. The returned text will not contain the stop sequence.
    store: bool | Any  # Whether or not to store the output of this chat completion request for use in our [model distillation](https://platform.openai.com/docs/guides/distillation) or [evals](https://platform.openai.com/docs/guides/evals) products.  Supports text and image inputs. Note: image inputs over 8MB will be dropped.
    stream: Any | Any  # If set to true, the model response data will be streamed to the client as it is generated using [server-sent events](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events/Using_server-sent_events#Event_stream_format). See the [Streaming section below](https://platform.openai.com/docs/api-reference/chat/streaming) for more information, along with the [streaming responses](https://platform.openai.com/docs/guides/streaming-responses) guide for more information on how to handle the streaming events.
    stream_options: ChatCompletionStreamOptions | Any  # Options for streaming response. Only set this when you set  .
    temperature: float | Any  # What sampling temperature to use, between 0 and 2. Higher values like 0.8 will make the output more random, while lower values like 0.2 will make it more focused and deterministic. We generally recommend altering this or   but not both.
    ...
    top_logprobs: float | Any  # An integer between 0 and 20 specifying the maximum number of most likely tokens to return at each token position, each with an associated log probability. In some cases, the number of returned tokens may be fewer than requested.   must be set to   if this parameter is used.
    top_p: float | Any  # An alternative to sampling with temperature, called nucleus sampling, where the model considers the results of the tokens with top_p probability mass. So 0.1 means only the tokens comprising the top 10% probability mass are considered.  We generally recommend altering this or   but not both.
    user: str
    verbosity: Any | Any | Any | Any  # Constrains the verbosity of the model's response. Lower values will result in more concise responses, while higher values will result in more verbose responses. Currently supported values are  ,  , and  .
...
class ChatCompletionStreamParams(TypedDict):
...
class ClusterForkInterfaceOptions(TypedDict):
    ...
    nativeId: ScryptedNativeId  # The native id of the mixin that is associated with this fork.
...
class Device(TypedDict):
    ...
    interfaces: list[str]
    name: str
    nativeId: ScryptedNativeId  # The native id that is used by the DeviceProvider used to internally identify provided devices.
    providerNativeId: ScryptedNativeId  # The native id of the hub or discovery DeviceProvider that manages this device.
...
class DeviceManifest(TypedDict):
    ...
    devices: list[Device]
    providerNativeId: ScryptedNativeId  # The native id of the hub or discovery DeviceProvider that manages these devices.
...
class ScryptedUserAccessControl(TypedDict):
    ...
    devicesAccessControls: list[ScryptedDeviceAccessControl] | Any  # If devicesAccessControls is null, the user has full access to all devices.
...
class ScryptedNativeId(TypedDict):
...
class DeviceProvider:
    ...
    async def getDevice(self, nativeId: ScryptedNativeId) -> Any:
        pass

    async def releaseDevice(self, id: str, nativeId: ScryptedNativeId) -> None:
        pass
...
class MixinProvider:
    ...
    async def canMixin(self, type: str, interfaces: list[str]) -> None | list[str] | Any | undefined:
        pass
...
class DeviceManager:
    ...
    def getDeviceConsole(self, nativeId: ScryptedNativeId = None) -> Console:
        pass

    def getDeviceLogger(self, nativeId: ScryptedNativeId = None) -> Logger:
        pass

    def getDeviceState(self, nativeId: ScryptedNativeId = None) -> DeviceState:
        pass

    def getDeviceStorage(self, nativeId: ScryptedNativeId = None) -> Storage:
        pass

    def getMixinConsole(self, mixinId: str, nativeId: ScryptedNativeId = None) -> Console:
        pass

    def getMixinStorage(self, id: str, nativeId: ScryptedNativeId = None) -> Storage:
        pass

    def getNativeIds(self) -> list[ScryptedNativeId]:
        pass

    async def onDeviceDiscovered(self, device: Device) -> str:
        pass

    async def onDeviceEvent(self, nativeId: ScryptedNativeId, eventInterface: str, eventData: Any) -> None:
        pass

    async def onDeviceRemoved(self, nativeId: ScryptedNativeId) -> None:
        pass
...
class EndpointManager:
    ...
    async def getAuthenticatedPath(self, nativeId: ScryptedNativeId = None) -> str:
        pass

    async def getCloudEndpoint(self, nativeId: ScryptedNativeId = None, options: Any = None) -> str:
        pass

    async def getCloudPushEndpoint(self, nativeId: ScryptedNativeId = None) -> str:
        pass

    async def getInsecurePublicLocalEndpoint(self, nativeId: ScryptedNativeId = None) -> str:
        pass
    ...
    async def getPath(self, nativeId: ScryptedNativeId = None, options: Any = None) -> str:
        pass

    async def getPublicCloudEndpoint(self, nativeId: ScryptedNativeId = None) -> str:
        pass

    async def getPublicLocalEndpoint(self, nativeId: ScryptedNativeId = None) -> str:
        pass

    async def getPublicPushEndpoint(self, nativeId: ScryptedNativeId = None) -> str:
        pass
...
class ClusterManager:
    ...
    def getClusterMode(self) -> Any | Any | undefined:
        pass
