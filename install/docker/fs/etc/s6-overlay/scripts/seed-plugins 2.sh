#!/bin/bash
# Scryvex Pro - First-boot plugin seeder
...
if [ -f "$SEED_FLAG" ]; then
    echo "[Scryvex Pro] Custom plugins already seeded, skipping."
    exit 0
fi
...
echo "Seeded by Scryvex Pro on $(date)" > "$SEED_FLAG"
echo "[Scryvex Pro] Plugin seeding complete!"
