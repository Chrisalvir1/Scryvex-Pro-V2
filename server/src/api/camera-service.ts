...
export class CameraService {
    ...
    async updateHomeKitMode(id: string, mode: Record<string, unknown>) {
        await this.pool.query(`UPDATE scryvex_core.cameras SET config = jsonb_set(config, '{homekit_stream_mode}', $1::jsonb), updated_at = NOW() WHERE id = $2`, [JSON.stringify(mode), id]);
    }
